...
#define DELETE_IF_NOT_ALREADY(node) \
if( (node)!=NULL && ! (node)->getDeleteStatus() ) \
    delete (node);
 ...
 *   History:
 * $Log: astnode.h,v $
 * Revision 1.7  2010/01/30 00:20:07  testadit
 * *** empty log message ***
 *
 ...
 * *** empty log message ***
 *
 * Revision 1.4  2001/12/04 20:58:18  madings
...
typedef struct
{
    ...
    unsigned int isCache    : 1;  // boolean: am I merely a node in cache and
                                  // not a real node in the tree?
...
{ 
  ...
  /** Every subtype must provide an unparse operation that
    ...
    * @param colSpacing - the number of additional spaces to pad
    *                 inside tabulated loops on output.
    *                 for the minimum, set it to 0.
    *                 The typical value is 2 (1 mandatory plus 2 more).
    */
  ...
  virtual ASTtype myType(void);
   ...
   //  getDeleteStatus()
  //---------------------------
  /** return true if this node is already in it's destructor.
    * When trying to delete anything, this check should be
    * used first to see if it's a good idea.  The macro
    * DELETE_IF_NOT_ALREADY does this for you.
    */
  virtual bool getDeleteStatus(void);
  ...
  /** <u>Overloaded Version</u>
    ...
    * @param searchFor - Look for this string as the tag name.
    */
  virtual List<ASTnode*> * searchByTag( const char *searchFor);
  ...
  //@{
	...
	* longer needed.
	*/
      virtual List<ASTnode*> * searchByTagValue( const string &tag, const char *value);

      /** <u>Overloaded Version</u>
	* Just calls the above version of this function.  There is no
	* need to override this function in subclasses of ASTnode.
	*/
      virtual List<ASTnode*> * searchByTagValue( const char *tag, const string &value);

      /** <u>Overloaded Version</u>
	* Just calls the above version of this function.  There is no
	* need to override this function in subclasses of ASTnode.
	*/
      virtual List<ASTnode*> * searchByTagValue( const char *tag, const char *value);
  //@}
  ...
  //@{
      ...
      virtual List<ASTnode*> * searchForTypeByTag(
	      ASTtype  type, const string   &tag);

      /** <u>Overloaded Version</u> */
      virtual List<ASTnode*> * searchForTypeByTag(
	      ASTtype  type, const char *tag);
  ...
  //@{
      ...
      virtual List<ASTnode*> * searchForTypeByTagValue(
	      ASTtype  type, const string   &tag, const string   &value);
	...
	*/
      virtual List<ASTnode*> * searchForTypeByTagValue(
	      ASTtype  type, const string    &tag, const char     *value );
	...
	*/
      virtual List<ASTnode*> * searchForTypeByTagValue(
	      ASTtype  type, const char     *tag, const string   &value);
	...
	*/
      virtual List<ASTnode*> * searchForTypeByTagValue(
	      ASTtype  type, const char    *tag, const char    *value );
  ...
  //@{
      ...
      /** These functions are used to some nodes in the
	* AST tree the ability to remember a comment to be
	* pasted into the file in front of that node.
	* <p>
	* In the interest of saving memory, these functions are
	* not implemented at all levels of the tree, although for
	* orthogonality they exist in the generic ASTnode parent
	* class, for most classes calling them will have no effect,
	* or will always return an empty string.
	*/
  //@}

  //-------------------------------------------
  // getIsCache() / setIsCache()
  //-------------------------------------------
  //@{
      ///
      const bool  getIsCache( void )
      {
	  return bitFields.isCache ;
      };
      ///
      void       setIsCache( bool isIt )
      {
	  bitFields.isCache = isIt;
      };
