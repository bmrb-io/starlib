...
/*********************************************************************
//  $Log: ast_working.h,v $
//  Revision 1.3  2001/12/04 20:58:17  madings
...
//
//  Revision 1.2  2001/11/13 20:30:29  madings
...
#endif
{
 ...
 public:
  ...
  //@{
      ...
      /** Changes the value of this string.  *DO NOT* inlcude
  ...
  ///
  virtual void Unparse(int indent) { Unparse( indent, 0, -1 ); };
  ...
  virtual void Unparse(int indent, int presize, int postsize);
   ...
   //  searchForType  //
  //--------------------------------------------------
  virtual List<ASTnode*> * DataValueNode::searchForType(
	     ASTtype    type,
	     int        delim
	     );
...
public:
  ...
  void debugPrint( void );
    ...
    /** This is designed to behave like the STL vector iterator.
      ...
      * expected to have access to the STL documentation descrtibing
      * vectors.
...
class DataLoopValListNode : public ASTnode, private ASTnodeWithPosBits
{
 public:
  ...
  Status getNextValue(DataValueNode* &value);
  void reset();
  ...
  //----------------------------------------------------------
  // Functions needed because this s a derivative of ASTnode:
  //----------------------------------------------------------
  virtual void     Unparse(int indent);
...
{
public:
  LoopValListNode(ASTlist<DataValueNode *> *L);
  LoopValListNode();
  virtual ~LoopValListNode();

  void reset();
  DataLoopValListNode::Status getNextValue(DataValueNode* & value);
...
public :
	...
	public:
	    ...
	    ///
	    iterator( const ASTlist<DataValueNode*>::iterator  &copyMe )
	    {
		vIter = copyMe;
	    }
	    ///
	...
	public:
	    ...
	    ///
	    const_iterator( const ASTlist<DataValueNode*>::const_iterator  &copyMe )
	    {
		vIter = copyMe;
	    }
	    ///
    ...
    ///
    iterator begin() { return myVals.begin(); }
    ///
    iterator end() { return myVals.end(); }
    ///
    const_iterator begin() const { return myVals.begin(); }
    ///
    const_iterator end() const { return myVals.end(); }
    ///
    size_type  size() const { return myVals.size(); }
    ///
    bool empty() const { return myVals.empty(); }
    ///
    reference operator[]( size_type n ) { return (myVals[n]); }
    ///
    const_reference operator[]( size_type n) const { return (myVals[n]); }
    ///
    reference front() { return myVals.front(); }
    ///
    const_reference back() const { return myVals.back(); }
    ///
    const_reference front() const { return myVals.front(); }
    ///
    reference back() { return myVals.back(); }
...
virtual void Unparse(int indent) { Unparse( indent, NULL, NULL ); };
...
private:
	...
	DataValueNode  &val );   // Value to insert.
...
public:
  /** Returns the length of the longest string in this object.
    ...
    * data value.
    */
  virtual int myLongestStr( void )  const { myDefList->myLongestStr(); };
