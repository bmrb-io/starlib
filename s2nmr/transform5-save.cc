	...
	iterVal[i] = tagToVal(currDicValList->Current()->myName());
		...
		if ( saveFrameCur->isOfType(ASTnode::SAVEFRAMENODE) )
		{
		    	...
		    	if ( astTagCur->isOfType(ASTnode::DATAITEMNODE) )
			{
		    	...
		    	{
       			     for ( ; 
			          ((astTagCur != NULL) && (astTagCur->myType() != ASTnode::DATALOOPNODE));
		       	     ...
		       	     if ( astTagCur != NULL )
			     {
			         ...
			         if (nmrTagPeer != NULL)
				 {
				     if( length > 2 && iterVal[2] == string("L") )
				     {
				 ...
				 // Check to see if the loop is empty.  If so, get rid of it:
				 if( nmrTagPeer != NULL )
				 {
				     // If the outermost nesting level of
				     // loop names is an empty list:
				     if(   (  ( (DataLoopNode*)nmrTagPeer )->getNames()[0]   )->size() == 0 )
				     {
					 delete ((DataLoopNode*)nmrTagPeer);
					 nmrTagPeer = NULL;
				     }
