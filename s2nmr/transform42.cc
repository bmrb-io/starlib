...
#include "transforms.h"
  ...
  ///////////////////////////////
 //    save_remove_2.1_tags   //
///////////////////////////////
void save_remove_version_2_1_tags( StarFileNode *input,
	                           StarFileNode *output,
		                   BlockNode    *blk,
		...
		else if( curThingy && curThingy->isOfType( ASTnode::DATALOOPNODE ) )
		{
			...
			for( j = 0; j < names->size() ; j++ )
			{
				...
				{   LoopRowNode *lrn = new LoopRowNode(true);
				    ...
				    break;  // With the mapping loop flag set to "L"
				            // there's no need to do more than one
					    // of the columns of a loop.
